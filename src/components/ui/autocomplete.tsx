...
>({
  ...
  const [fetchPreloadData, { data: preloadData, loading: preloadLoading }] =
    useLazyQuery(query || EMPTY_QUERY, { fetchPolicy: "no-cache" });
  ...
  useEffect(() => {
    ...
    const shouldRun = isFirstDefaultValueSet || syncWithDefaultValue;
    ...
    if (valueToUse !== undefined && currentOptions.length > 0) {
      ...
      // Handle reset when syncWithDefaultValue is true and value is empty
      setSelectedOptions([]);
      setSearch("");
    }
  ...
  useEffect(() => {
    if (open && query && variables) {
      // In asSelect mode, always load data when opening (regardless of existing options)
      // In regular mode, only load if we have no options and no search term
      const shouldLoad = asSelect || (graphqlOptions.length === 0 && !search);
          ...
          multiple ? (
            ...
            >
              ...
              <input
                ...
                readOnly={
                  asSelect || (preloadLoading && query && !!defaultValue)
                }
